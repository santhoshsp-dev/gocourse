...
So these contexts are kind of objects.
These are instances of structs.
And instances of structs are objects.
Go does not have object oriented programming.
It doesn't have classes.
It has structs.
So when we are creating an instance of a struct it is an object.
So contexts are objects that we are creating in our code.
*/
// func main() {
// Step: 1)
...
// 	ctx = context.WithValue(ctx, "requestID", "jnk345bkbkjhl")
...
func main() {
	ctx := context.Background()
	// ctx, cancel := context.WithTimeout(ctx, 1*time.Second) // timer of the context starts here. You have this specified time duration to use this context. After this time duration, the context will send a cancelation signal.
