...
func XSSMiddleware(next http.Handler) http.Handler {
	println("****** Initializing SXXMiddleware")
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("Content-Type") == "application/json" {
			if r.Body != nil {
				...
				// cannot use bytes.NewReader([]byte(bodyString)) (value of type *bytes.Reader) as io.ReadCloser value in assignment: *bytes.Reader does not implement io.ReadCloser (missing method Close) so we need to use io.NopCloser
...
func sanitizeValue(data interface{}) interface{} {
	...
	case []interface{}:
		...
		return v
	default:
		return v // Return as it is unsupported
	}
}
